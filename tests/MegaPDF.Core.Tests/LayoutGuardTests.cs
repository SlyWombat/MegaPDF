...
/// <summary>
/// PDFium's content writer drops character spacing, word spacing, scaling and rise when
/// it rewrites a text object (#118), so an edit on such a page would quietly move text
/// the person never touched. The engine must refuse and leave the page exactly as it was.
/// </summary>
public sealed class LayoutGuardTests : IDisposable
{
    ...
    [Fact]
    public void TextUnderCharacterSpacing_IsNotEditable_AndAnEditIsRefusedWithThePageUnchanged()
    {
        ...
        Assert.Equal(line.Runs[0].Text, run.Text);
        Assert.Equal(line.Runs[0].Bounds, run.Bounds);
    }

    [Fact]
    public void DeletingTextUnderCharacterSpacing_IsRefusedToo()
    {
        using var doc = Open(Pdf("BT /F1 24 Tf 4 Tc 72 700 Td (Spaced report) Tj ET"), "spaced-delete.pdf");
        ...
        using var after = doc.GetPage(0);
        Assert.Single(after.GetTextRuns());
    }
