...
final class UserUnitTests: XCTestCase {
    ...
    func testPageMeasuresInPointsAndContentIsWhereItIsDrawn() async throws {
        ...
        let fields = try await engine.formFields(doc, pageIndex: 0)
        let box = try XCTUnwrap(fields.first)
        XCTAssertEqual(box.rect.left, 100, accuracy: 0.1)
        XCTAssertEqual(box.rect.bottom, 260, accuracy: 0.1)
        XCTAssertEqual(box.rect.right, 115, accuracy: 0.1)
        XCTAssertEqual(box.rect.top, 275, accuracy: 0.1)
        try await engine.clickAt(doc, pageIndex: 0, x: 107.5, y: 267.5)
        let ticked = try await engine.formFields(doc, pageIndex: 0)
        XCTAssertEqual(ticked.first?.isChecked, true, "a tap at the checkbox's centre ticks it")
    ...
    func testPlacementsLandWhereAsked() async throws {
        ...
        var box = try XCTUnwrap(boxes.first)
        ...
        let stamp = try XCTUnwrap(stamps.first { $0.id == "sig:unit" })
