...
///
/// The pages scroller is a tab stop. Focus arriving there lands on the first
/// interactive region of the page on screen (the last, arriving with Shift+Tab);
/// Tab and Shift+Tab walk the regions in reading order across pages; Enter or Space
/// activates the focused region through exactly the routing a tap uses, aimed at
/// its centre; Escape hands focus back to the toolbar. Past the last region Tab
/// carries on to the next control, so nobody is trapped on the page.
///
/// Keyboard focus itself stays on the scroller rather than moving into the page.
/// A page's container is rebuilt on every render, and focus inside it would be
/// dropped each time; the ring is drawn on the page instead, named for UI
/// Automation, and each move is announced to Narrator as a notification.
/// </summary>
public sealed partial class MainWindow
{
    ...
    /// Called from the root PreviewKeyDown, which decides before any await, so the
    ...
    private async Task StepPageFocusAsync(bool forward)
    {
        ...
        ViewModel.ClearPageFocus();
        var options = new FindNextElementOptions { SearchRoot = Content.XamlRoot.Content };
    ...
    private void UpdateFocusRing()
    {
        ...
        ring.Children.Add(new Border { Style = (Style)RootGrid.Resources["PageFocusRingOuterStyle"] });
        ring.Children.Add(new Border { Style = (Style)RootGrid.Resources["PageFocusRingInnerStyle"] });
