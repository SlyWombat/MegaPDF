...
class PdfEngineTest {
    ...
    @Test
    fun opensAndReportsGeometry() = runBlocking {
    ...
    @Test
    fun saveRoundTrips() = runBlocking {
        val doc = engine.open(fixtureBytes())
    ...
    @Test
    fun invalidBytesThrowLoadException() = runBlocking {
        try {
    ...
    @Test
    fun concurrentRendersAreSerialized() = runBlocking {
        val doc = engine.open(fixtureBytes())
        try {
            val page = doc.openPage(0)
            try {
